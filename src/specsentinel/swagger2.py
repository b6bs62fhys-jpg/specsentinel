...
def translate(data: dict) -> tuple[dict, dict]:
    ...
    for path, item in (data.get("paths") or {}).items():
        ...
        if reason is None and has_get:
            new_operation, reason = translate_operation(operation, global_produces)
    ...
    if isinstance(headers, dict):
        ...
        for name, header in headers.items():
            ...
            if header.get("type") == "file":
                return {}, "Swagger 2.0 'type: file' is not supported"
            new_header = {"schema": rewrite_refs(_schema_of(header))}
...
def _schema_of(source: dict) -> dict:
